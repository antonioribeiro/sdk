...
use PragmaRX\Sdk\Services\FacebookMessenger\Data\Entities\FacebookMessengerFile;
...
use PragmaRX\Sdk\Services\FacebookMessenger\Data\Entities\FacebookMessengerVenue;
use PragmaRX\Sdk\Services\FacebookMessenger\Data\Entities\FacebookMessengerVideo;
use PragmaRX\Sdk\Services\FacebookMessenger\Data\Entities\FacebookMessengerVoice;
use PragmaRX\Sdk\Services\FacebookMessenger\Data\Entities\FacebookMessengerAudio;
...
use PragmaRX\Sdk\Services\FacebookMessenger\Data\Entities\FacebookMessengerSticker;
use PragmaRX\Sdk\Services\FacebookMessenger\Data\Entities\FacebookMessengerContact;
use PragmaRX\Sdk\Services\FacebookMessenger\Data\Entities\FacebookMessengerDocument;
use PragmaRX\Sdk\Services\FacebookMessenger\Data\Entities\FacebookMessengerChatType;
use PragmaRX\Sdk\Services\FacebookMessenger\Data\Entities\FacebookMessengerLocation;
...
class FacebookMessenger
{
    ...
    {
        $photo = FacebookMessengerPhoto::createOrUpdate(
            $photo,
        ...
        {
            event(new FacebookMessengerPhotoWasCreated($photo, $bot));
        }
     ...
     * @return mixed
     */
    private function downloadFacebookMessengerFile($document, $bot, $hasThumb = false)
    {
    ...
    {
        return $this->downloadFacebookMessengerFile($voice, $bot, false);
    }

    public function downloadDocument($document, $bot)
    {
        return $this->downloadFacebookMessengerFile($document, $bot, true);
    }
    ...
    public function downloadUserAvatar($user, $bot = null)
    {
        ...
        $data = [
            'user_id' => $user->facebook_messenger_id
        ];

        $result = FacebookMessengerService::getUserProfilePhotos($data)->getResult();
        ...
        // FacebookMessenger may have too many photo profiles
        // Download only the first photo
        $key = 0;

        if ($photos && $fileName = $this->downloadAndMakeFileName($photos[$key]))
        {
            $photos[$key]['facebook_messenger_file_id'] = $photos[$key]['file_id'];
    ...
    {
        return $this->downloadFacebookMessengerFile($photo, $bot, false);
    }

    private function extractDocument($document)
    {
        return [
            'file_id' => $document->facebook_messenger_file_id,
    ...
    private function extractPhotos($photos)
    {
        ...
        if ($photos instanceof FacebookMessengerPhoto)
        {
            $photos = [[
                'file_id' => $photos->facebook_messenger_file_id,
                'width' => $photos->width,
    ...
    {
        $audio = FacebookMessengerAudio::createOrUpdate(
            [
                'facebook_messenger_file_id' => array_get($audio, 'file_id'),
        ...
        {
            event(new FacebookMessengerAudioWasCreated($audio, $bot));
        }
    ...
    private function firstOrCreateChat($chat, $bot)
    {
        return FacebookMessengerChat::createOrUpdate(
            [
                'facebook_messenger_id' => $chat['id'],
                'bot_id' => $bot->id,
                'facebook_messenger_chat_type_id' => $this->firstOrCreateType($chat['type'])->id,
    ...
    private function firstOrCreateDocument($document, $bot)
    {
        ...
        $document = FacebookMessengerDocument::createOrUpdate(
            [
                'facebook_messenger_file_id' => array_get($document, 'file_id'),
        ...
        {
            event(new FacebookMessengerDocumentWasCreated($document, $bot));
        }
    ...
    private function firstOrCreateLocation($location)
    {
        ...
        return FacebookMessengerLocation::firstOrCreate($location);
    }
    ...
    private function firstOrCreateMessage($data, $bot)
    {
        ...
        return FacebookMessengerMessage::createOrUpdate(
            [
                'facebook_messenger_message_id' => $data['message_id'],
            ...
            ['facebook_messenger_message_id', 'chat_id']
        );
    }
    ...
    private function firstOrCreateSticker($sticker, $bot)
    {
        ...
        return FacebookMessengerSticker::createOrUpdate(
            [
                'facebook_messenger_file_id' => array_get($sticker, 'file_id'),
    ...
    {
        return FacebookMessengerChatType::firstOrCreate(['name' => $type]);
    }

    private function firstOrCreateUser($user, $bot)
    {
        $user = FacebookMessengerUser::createOrUpdate(
            [
                'facebook_messenger_id' => array_get($user, 'id'),
    ...
    private function firstOrCreateVenue($data)
    {
        ...
        return FacebookMessengerVenue::firstOrCreate(
            [
    ...
    private function firstOrCreateVideo($video, $bot)
    {
        ...
        $video = FacebookMessengerVideo::createOrUpdate(
            [
                'facebook_messenger_file_id' => array_get($video, 'file_id'),
        ...
        {
            event(new FacebookMessengerVideoWasCreated($video, $bot));
        }
    ...
    {
        $voice = FacebookMessengerVoice::createOrUpdate(
            [
                'facebook_messenger_file_id' => array_get($voice, 'file_id'),
        ...
        {
            event(new FacebookMessengerVoiceWasCreated($voice, $bot));
        }
    ...
    {
        $facebookMessengerFile = FacebookMessengerFile::where('facebook_messenger_file_id', $file['file_id'])->first();

        if ($facebookMessengerFile)
        {
            return $facebookMessengerFile->fileName;
        }
        
        if (! $facebookMessengerFile = FacebookMessengerService::getFile($file))
        {
            return null;
        }

        $fileName = FacebookMessengerService::downloadFile(
            FacebookMessengerService::makeFileUrl($file, $facebookMessengerFile->getFilePath()),
            isset($file['width']) ? $file['width'] : null,
            isset($file['height']) ? $file['height'] : null
        );

        FacebookMessengerFile::create([
             'facebook_messenger_file_id' => $file['file_id'],
    ...
    private function makePhotoArray($photo, $idColumnName = 'facebook_messenger_file_id', $idColumnValue = null)
    {
